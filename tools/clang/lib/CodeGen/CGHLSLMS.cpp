...
unsigned CGMSHLSLRuntime::ConstructStructAnnotation(DxilStructAnnotation *annotation,
  ...
  // excess fields if it is a union. As a result we must construct the annotation
  ...
  if (RD->isUnion() && !RD->field_empty()) {
    ...
    for (FieldDecl *Field : RD->fields()) {
      ...
      // Process field to make sure the size of field is ready.
      unsigned arrayEltSize = 0;
      unsigned size = AddTypeAnnotation(Field->getType(), dxilTypeSys, arrayEltSize);
      // Align offset.
      if (size) {
